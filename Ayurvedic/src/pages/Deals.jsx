...
const Deals = () => {
  return (
    <div className="h-[90vh] w-full p-10">
      ...
      <div className="h-[60vh] flex justify-evenly w-full">
        {
          saman.map((product) => (
            <Link to={`/product/${product.id}`} key={product.id} className="h-[45vh] w-[45%] p-4 flex pt-6 border-2 border-green-500 rounded-md hover:bg-slate-50 transition-all">
          <img
            src={product.imageUrl}
            alt=""
            className="h-[95%] w-[60%] object-cover rounded-lg"
          />
